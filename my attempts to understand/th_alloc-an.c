...
void free(void *ptr) {
  ...
  for (; i < (bkeep->level+4); i++) {
    div *= 2;
  }

  test = (SUPER_BLOCK_SIZE/div)-1;
  ...
  while (levels[bkeep->level].whole_superblocks > RESERVE_SUPERBLOCK_THRESHOLD) {
    ...
    //// basically freeing an object 1) so freeing hte superblocks one can have: superbloacks LL
    ///example: going down blocks, so delete empty blocks each bkeep->next
    /// if delete a middle, reset pointer 
      munmap(bkeep, levels[bkeep->level].whole_superblocks);
      struct object *next = bkeep->levels[bkeep->level].whole_superblocks;
      rv = next;
      bkeep->levels[bkeep->level].whole_superblocks = next->next;
